...
/**
 * This is an generic Queue implementation using a circular array. This class
 * has been optimized for algorithmic runtime, memory usage, and memory
 * throughput.
 *
...
public class Queue<E> {
	...
	/**
	 * @param element
	 *            Element to add to the queue if not full.
	...
	public boolean enqueue(E element) {
		if (((rearElementIndex + 2) % maxSize) == frontElementIndex) {
			throw new IllegalStateException("Bounded queue is full at size = " + this.length()
					+ ". Call dequeue() before recalling " + "enqueue()");
		}
	...
	public E dequeue() {
		if (length() == 0) {
			throw new IllegalStateException("Bounded queue is currently empty. " + "Call enqueue() before dequeue()");
		}
